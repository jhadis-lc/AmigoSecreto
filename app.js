...
function agregarAmigo() {
    ...
    amigos.push();
    document.getElementById('amigo').remove;
    alert('q funcione esto');
};
...
function validarInput(nombreAmigo) {
    let nombreAmigo = document.getElementsByClassName('input-name').value;
    if (nombreAmigo == 'Jhadis') {
...
function sortearAmigo() {
    ...
    let sortear = Math.floor(Math.random()*10)+1;
    return sortear;
    document.getElementById('button-draw');
    innerHTML.
    alert('q funcione esto');
}
